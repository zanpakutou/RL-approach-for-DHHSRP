...
def run_offline_DH(no: int):
    env = PatientRequest()
    env.make(instance_dir + str(no) + ".in", instance_dir + "/../../context.in",nb_weeks=nb_simulation_week)
    sched = Schedule(env)
    ...
    for week in range(env.nb_weeks):
        ...
        while is_insertable:
            ...
            for _request in _requests:
                ...
                if valid == True:
                    heuristic = min_cost_insertion[0][0] / reward(
                        args.obj, _requests[best_index]
                    )
...
def run_SBA_greedy(
    ...
    sba = SBA(
        sched,
        PatientGenerator(mode=args.instance_type),
...
def run_stable_baselines(
    ...
    config = Config()
    env_type = TimeLimit(
        DHHSRP(instance_dir, reward_type=0, nb_nurse=args.nb_nurse),
...
if __name__ == "__main__":
    ...
    header = []
    if (args.switch_CH == 1):
        header = ["ch_sum_travel","ch_avg_travel","ch_dev_travel","ch_sum_service","ch_avg_service","ch_dev_service","ch_visit","total_request","ch_patient",\
            "sba_ch_sum_travel","sba_ch_avg_travel","sba_ch_dev_travel","sba_ch_sum_service","sba_ch_avg_service","sba_ch_dev_service","sba_ch_visit",\
            "total_request","sba_ch_time","sba_ch_valid","sba_ch_patient","rl_sum_travel","rl_avg_travel","rl_dev_travel","rl_sum_service","rl_avg_service",\
            "rl_dev_service","rl_visit","total_request","rl_time","rl_valid","rl_patient","id"
        ]
    else:
        header = ["dho_sum_travel","dho_avg_travel","dho_dev_travel","dho_sum_service","dho_avg_service","dho_dev_service","dho_visit","total_request","dho_patient",\
            "dh_sum_travel","dh_avg_travel","dh_dev_travel","dh_sum_service","dh_avg_service","dh_dev_service","dh_visit","total_request","dh_patient",\
            "sba_dh_sum_travel","sba_dh_avg_travel","sba_dh_dev_travel","sba_dh_sum_service","sba_dh_avg_service","sba_dh_dev_sercive","sba_dh_visit",\
            "total_request","sba_dh_time","sba_dh_valid","sba_dh_patient","id"
        ]
    with open(
        filename + ".csv", "w",
        ...
        for no in range(args.instance_id, args.instance_id + 1):
            ...
            xxx = "../stable_baselines/30_03_20/visit/"
            yyy = (
                args.instance_type
                + "_"
                + str(args.arr_rate)
