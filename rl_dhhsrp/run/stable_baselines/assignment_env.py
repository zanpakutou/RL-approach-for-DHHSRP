...
class DHHSRP(gym.Env):
    ...
    def __init__(
        ...
        self.env.make(
            self.instance_dir + str(0) + ".in",
            self.instance_dir + "/../../context.in",
            nb_weeks=nb_weeks,
        )
    ...
    def reset(self):
        ...
        self.env.make(
            self.instance_dir + str(instance_no) + ".in",
            self.instance_dir + "/../../context.in",
            nb_weeks=nb_weeks,
        )
        ...
        for week in range(0, 4):
            for day in range(self.env.day_per_week):
                for request in self.requests[week][day]:
                    current_time = (week, day, request.current_time)
                    (valid, min_cost_insertion) = self.sched.check_feasible(
                        request, current_time, weekly_deadline=True
                    )
                    if valid == True:
                        self.sched.accept_checked_request(
                            request, min_cost_insertion, weekly_deadline=True
                        )
                        initial_state = self.feature_extractor.get_feature(
                            request=request,
                            current_time=current_time,
                            is_post_state=True,
                            obj = self.reward_type
                        )
                        self.request_position = (
                            week, day, len(self.requests[week][day]) - 1,
                        )
    ...
    def step(self, action):
        ...
        if self.is_post_state == True:
            ...
            (valid, min_cost_insertion) = self.sched.check_feasible(
                ...
                capacity_heur=self.cap_heur,
            )
            obs = self.feature_extractor.get_feature(
            ...
            if action == self.REJECT or valid == False:
                ...
                nurse = action - 1
                (_valid, min_cost_insertion) = self.sched.check_feasible(
                    request,
                    self.current_time,
                    spec_nurse=nurse,
                    weekly_deadline=True,
