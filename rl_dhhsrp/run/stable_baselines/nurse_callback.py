            ...
            for request in requests[week][day]:
                ...
                if valid == True:
                    ...
                    if action == 0 and week > 3:
                        ...
                        check = sched.accept_request(
                            request, current_time, spec_nurse=action - 1, weekly_deadline=True, capacity_heur = cap_heur
                        )
                        if (check):
                            ans_rl = ans_rl + reward(obj, request)
                        else:
                            invalid_nurse = invalid_nurse + 1
    if (epsilon < 0.02 and invalid_nurse > 0):
        print(invalid_nurse)
    return ans_rl
