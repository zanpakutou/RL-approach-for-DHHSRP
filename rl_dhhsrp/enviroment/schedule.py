...
def best_case_func(p, s):
    ...
    case2 = (s.st - p.st - delta * np.ceil(distance(s.pos, p.pos)
             / delta)) // duration - 1
    return min(case1, case2)


def flexible_capacity_func(p, s):
    return (s.st - p.st) // delta - (duration // delta + 1) \
        * (best_case_func(p, s) + 1)
...
class Route:
    ...
    def __init__(self, depot, working_tw):
        self.visit = [Visit(depot, working_tw[0], working_tw[0]),
    ...
    def insert(
        ...
        for pos in range(1, len(self.visit)):
            ...
            insert_cost = distance(prev.pos, position) \
                + distance(position, next_.pos) - distance(prev.pos,
                    next_.pos)
        ...
        if save_pos > -1:
            if checking == False:
                self.visit.insert(save_pos, Visit(position, start_time,
                                  end_time))
            return (min_cost, )
        else:
            return (-1, )
    ...
    def capacity_criteria(
        ...
        for pos in range(1, len(self.visit)):
            ...
            best_case = best_case_func(p, s) - best_case_func(p, q) \
                - best_case_func(q, s)
...
class Schedule:
    ...
    def check_feasible(
            ...
            for start_week in range(current_time[0] + 1, max_start_week + 1):
                ...
                for time in range(0, 1440, 15):
                    ...
                    for nurse in range(0, self.nb_nurses):
                        ...
                        for week in range(start_week, start_week
                            ...
                            for day in pattern:
                                ...
                                nurse_total_cost = \
                                    tuple(map(operator.add, nurse_total_cost, cost))

                        if capacity_heur == True:
                            nurse_total_cost = (nurse_total_cost[0],
                                    nurse_total_cost[1],
                                    self.count_accept_pat[pat_id],
                                    nurse_total_cost[3])
                        if nurse_is_ok and nurse_total_cost \
                            < min_cost_insertion[0]:
                            min_cost_insertion = (
                                nurse_total_cost,
                                nurse, time,
        ...
        if capacity_heur == True:
            min_cost_insertion = ((min_cost_insertion[0][3], ), min_cost_insertion[1], 
    ...
    def accept_request(
        ...
        (ok, min_cost_insertion) = self.check_feasible(request,
                current_time, spec_nurse,
        ...
        self.count_accept_pat[pat_id] = self.count_accept_pat[pat_id] \
            + 1
        for week in range(start_week, start_week
                          + request.require_time[0]):
            for day in pattern:
            ...
            for day in range(len(self.planned_routes[0][week])):
                for nurse in range(self.nb_nurses):
                    ...
                    for pos in range(1, len(route.visit)):
                        ...
                        if prev.ed + distance(prev.pos, curr.pos) \
                            > curr.st:
                            return False
        return True
